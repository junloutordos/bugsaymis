...
# macOS ships bash 3.2 (no associative-array support, added in bash 4.0) — declare -A here
# fails with a cryptic "invalid arithmetic operator" error, found by actually running this
# on-device (not obvious from reading the script). Use a plain "filename:size" list instead.
