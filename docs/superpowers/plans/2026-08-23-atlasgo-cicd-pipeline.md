...
- **UPDATE 2026-08-24 ~10:57 — manual iOS distribution succeeded, real signal for the CI hang.** With CI/CD debugging parked, ran the same `ios/fastlane/Fastfile` `release` lane locally on this dev Mac (Xcode 26.5) instead of GitHub Actions. It worked essentially cleanly: `flutter build ios` in 9.7s, full archive+sign+export in well under a minute, **no hang at all** — the exact "Run Script"/xcode_backend.sh phase that hung for 30+ minutes on every CI attempt completed in seconds here. Only two trivial local-environment gaps got in the way, both fixed: (1) `xcpretty` wasn't installed on this Mac (`gem install xcpretty --user-install`, since passwordless sudo isn't available locally the way it is on CI runners) — a plain `command not found`, not a hang; (2) a one-time race where `Dir.glob` checked for the exported `.ipa` a beat before it finished writing to disk (fixed permanently with a short retry loop, committed). Final upload done via a one-off `fastlane run upload_to_testflight` pointed at the already-built IPA (avoided rebuilding) — `Successfully uploaded the new binary to App Store Connect`. AtlasGo v1.3.0 build 4 is now processing in TestFlight.
  - **This is a strong signal for resuming the CI hang investigation**: since the identical Fastfile/build logic works cleanly on a real Mac, the hang is very likely specific to something about the *GitHub Actions macos-15 runner environment* itself (network restrictions, a different/older Xcode 16 point release, sandbox policy differences, or resource constraints) rather than anything wrong in this repo's fastlane setup. Worth trying when picking this back up: pin a specific Xcode version explicitly via `xcode-select`/`maxim-lobanov/setup-xcode` instead of whatever macos-15 defaults to, since this Mac's Xcode 26.5 is far newer than any GH runner will have.
  - Manual Android distribution also done the same session: local `flutter build apk --release` (v1.3.0+4), uploaded to S3, backend `mobile_version` bumped to 1.3.0 and deployed to production.
- **Task 11 STATUS as of 2026-08-24 ~02:00 — paused for the night, resume here:**
  - **Android: fully proven, working.** `shorebird release android` builds, signs, and publishes a real Shorebird release every run (confirmed repeatedly: 1.3.0+5, +8, +11, +17...). Only failure is `upload_to_play_store` hitting the known, already-accepted account-eligibility precondition gate (not a bug — see the dedicated bullet below). Nothing left to do here except wait for the account to clear Google's testing threshold.
  - **iOS: three real bugs found and fixed** (Bundler env leak breaking the CocoaPods false-broken check, hardcoded provisioning profile name needing `update_code_signing_settings`, Xcode 15→16 runner switch for Swift 6). **One unresolved issue remains**, below.
  - **Unresolved: `build_app` (gym/xcodebuild archive) hangs indefinitely** at the exact same point every time — `[Runner] Running script Run Script` (Flutter's `xcode_backend.sh build`, invoked as a nested Xcode build phase). Confirmed via 3 independent 30-minute-capped runs, always the identical stall point, never progressing or erroring. Three targeted fixes tried and **none worked**:
    1. `Bundler.with_original_env` around the direct `flutter build ios` call (this one *did* fix the earlier, different CocoaPods-broken failure — it's a real, working fix, just not for this hang).
    2. `Bundler.with_original_env` around `build_app` itself too (theory: the nested Flutter invocation inside xcodebuild's Run Script phase inherits the same leaked bundler env) — hung identically.
    3. `xcargs: "ENABLE_USER_SCRIPT_SANDBOXING=NO"` on `build_app` (theory: Xcode 16's stricter Archive-time script sandboxing blocks something the nested Flutter script needs, e.g. network) — hung identically.
    - **What's confirmed:** the plain `build` action (`flutter build ios`) runs this *same* xcode_backend.sh script successfully; only the `archive` action (via `build_app`) hangs on it. So something specific to Archive-time execution of this script is the real cause — not yet identified.
    - **Next things to try, not yet attempted:** (a) reproduce interactively on a real Mac with Xcode 16 installed, where the hung process can actually be inspected (Activity Monitor / `sample <pid>` / `lldb -p <pid>`) instead of guessing blind against 30-minute CI cycles; (b) check whether `xcode_backend.sh`'s `build` verb behaves differently when `ACTION=install`/`ACTION=archive` is set in its environment (Xcode sets `$ACTION` per build action, and the script may branch on it — worth reading `$FLUTTER_ROOT/packages/flutter_tools/bin/xcode_backend.sh` directly rather than treating it as a black box); (c) try `skip_build_archive: false` alternatives or building the archive via a raw `xcodebuild archive` shell command outside fastlane entirely, to see if fastlane's `build_app` wrapper itself is part of the problem; (d) add explicit verbose/xcbeautify-disabled logging (`xcodebuild -verbose` via `gym`'s `xcodebuild_command` override) to see exactly what the script is doing (or not doing) when it hangs, since normal gym output goes silent during this phase.
    - A 30-minute `timeout-minutes` guard is in place on both jobs so a hang can never again run to GitHub's 6-hour default and waste large amounts of Actions time unnoticed.
- **Task 11 iOS build blockers, two distinct real bugs found via a real tagged release (not visible from local dev or planning):**
  1. **Shorebird's own `shorebird release ios` wrapper falsely reports "CocoaPods is installed but broken. Skipping pod install."** even when CocoaPods is fully functional — confirmed via a diagnostic step that ran plain `flutter build ipa` (bypassing Shorebird entirely) in the identical environment: `pod install` completed successfully in 44.6s, no broken-check warning at all. Tried and ruled out as fixes: brew vs. gem install, CocoaPods version (1.16.2 and 1.15.2), explicit `--flutter-version` pin, disabling Swift Package Manager integration, switching to macos-15/Xcode 16 (fixed a separate real problem, see #2, but not this one). This is upstream Shorebird behavior (see their issue #3049, closed but the "fix" — reinstall CocoaPods — doesn't address a subprocess-environment-isolation bug), not something fixable from this repo's config.
     - **Decision (user, 2026-08-23): drop Shorebird OTA patching for iOS specifically, keep it for Android.** The iOS fastlane lane no longer calls `shorebird release ios` at all — it builds via plain `flutter build ios --no-codesign` + fastlane's own `build_app` (gym), which correctly consumes match's `MATCH_PROVISIONING_PROFILE_MAPPING` lane context for manual signing, then uploads to TestFlight as originally designed. This is a real, deliberate scope reduction from the approved spec (which specified Shorebird for both platforms) — see the spec's Decision 6 update. `shorebird-patch.yml`'s iOS branch will not work for any iOS version released this way, since no Shorebird release record exists for it; that workflow now effectively only serves Android until Shorebird's bug is fixed upstream, at which point iOS Shorebird support can be re-added by reverting this change.
  2. **The real, separate blocker**: a Firebase/GoogleUtilities pod uses Swift 6 syntax (`sending` keyword, `AccessLevelOnImport`) that Xcode 15.4 — the default on GitHub's `macos-14` runner — cannot compile. Confirmed via the same diagnostic's actual compiler errors once it got past the (non-)CocoaPods step. Fixed by switching `release-ios`'s `runs-on` from `macos-14` to `macos-15` (default Xcode 16+, full Swift 6 support) in both `release.yml` and `shorebird-patch.yml`. This also updates the spec's stated runner choice.
