...
Route::middleware(['web', 'auth', 'verified', 'permission:quiz.manage|quiz.view_all'])
    ...
    ->group(function () {
        ...
        Route::post('/{quiz}/questions/reorder', [QuizController::class, 'reorderQuestions'])->name('questions.reorder');
        Route::post('/{quiz}/questions/{question}', [QuizController::class, 'updateQuestion'])->name('questions.update');
        Route::delete('/{quiz}/questions/{question}', [QuizController::class, 'destroyQuestion'])->name('questions.destroy');

        Route::post('/{quiz}/sessions', [QuizSessionController::class, 'store'])->name('sessions.store');
    });
...
Route::middleware(['web'])
    ...
    ->name('quiz.')
    ->group(function () {
        Route::get('/{quiz}/cover', [QuizController::class, 'showCoverImage'])->name('cover');
