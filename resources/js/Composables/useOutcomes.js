...
export function useOutcomes(props) {
  ...
  // Computed: filtered and paginated outcomes
  ...
  const totalPages = computed(() =>
    Math.ceil(outcomesList.value.length / perPage)
  )
      ...
      if (modalMode.value === "create") {
        router.post(url, form.value, {
          onSuccess: async (page) => {
            ...
            await Swal.fire(
              "Success",
              "Outcome has been added successfully",
              "success"
            )
        ...
        router.put(`${url}/${form.value.id}`, form.value, {
          onSuccess: async (page) => {
            closeModal()
            const index = outcomesList.value.findIndex(
              (o) => o.id === form.value.id
            )
            if (index !== -1)
              outcomesList.value[index] =
                page.props.outcome ?? { ...form.value }
            await Swal.fire(
              "Updated",
  ...
  const deleteOutcome = async (outcome) => {
    ...
    if (result.isConfirmed) {
      router.delete(`/agency-outcomes/${outcome.id}`, {
        onSuccess: async () => {
          outcomesList.value = outcomesList.value.filter(
            (o) => o.id !== outcome.id
          )
