  ...
  <AdminLayout :title="`IPCR Monitoring: ${ipcr.title}`">
    <div>
      ...
      <div class="bg-white p-4 rounded-lg shadow" id="ipcr-printable">
        ...
        <!-- Coaching & Mentoring Journal — carried over from the Division Chief's submission -->
        <div v-if="coachingSessions.length" class="mt-6 space-y-6">
          <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">Performance Monitoring and Coaching Journal</h2>

          <div v-for="session in coachingSessions" :key="session.id" class="border rounded-lg overflow-hidden">
            <table class="min-w-full border text-sm border-collapse">
              <tbody>
                <tr><td colspan="4" class="border px-3 py-2 font-semibold text-gray-800">Performance Monitoring and Coaching Journal</td></tr>
                <tr><td colspan="4" class="border px-3 py-2">Rating Period: {{ ipcr.period?.label ?? ipcr.rating_period }}</td></tr>
                <tr><td colspan="4" class="border px-3 py-2">Name of Campus: Philippine Science High School &ndash; Caraga Region Campus</td></tr>
                <tr><td colspan="4" class="border px-3 py-2">Name of Division/Unit: {{ employee.division?.name ?? '—' }}</td></tr>
                <tr><td colspan="4" class="border px-3 py-2">Name of Personnel: {{ employee.name }}, {{ employee.position }}</td></tr>
                <tr><td colspan="4" class="border px-3 py-2">Immediate Supervisor: {{ supervisor?.name ?? '—' }}</td></tr>
                <tr>
                  <td class="border px-3 py-2 font-medium w-32">Activity</td>
                  <td class="border px-3 py-2">{{ activityTypeLabel(session.activity_type) }}</td>
                  <td class="border px-3 py-2 font-medium w-32">Mechanism</td>
                  <td class="border px-3 py-2">{{ mechanismLabel(session.mechanism) }}</td>
                </tr>
                <tr>
                  <td class="border px-3 py-2 font-medium">Date of Meeting</td>
                  <td class="border px-3 py-2">{{ formatSessionDate(session.meeting_date) }}</td>
                  <td class="border px-3 py-2 font-medium">Channel</td>
                ...
                <tr>
                  <td class="border px-3 py-2 font-medium align-top">Remarks</td>
                  <td colspan="3" class="border px-3 py-2 whitespace-pre-wrap">{{ session.remarks }}</td>
                </tr>
                <tr>
                  <td class="border px-3 py-3 text-center" colspan="2">
                    <br/>
                    <b style="text-transform: uppercase;">{{ session.conducted_by_name }}</b><br/>
                    <small>Conducted by &mdash; {{ formatSessionDate(session.conducted_at) }}</small>
                  </td>
                  <td class="border px-3 py-3 text-center" colspan="2">
                    <br/>
                    <b style="text-transform: uppercase;">{{ session.noted_by_name || '—' }}</b><br/>
                    <small>Noted by &mdash; {{ formatSessionDate(session.noted_at) }}</small>
                  </td>
                </tr>
              </tbody>
            </table>
