...
// Props from backend (UserController@index)
const props = defineProps({
  users: Array,
  roles: Array,
})

// Composable: all user logic
const {
  usersList,
  rolesList,
  showModal,
      ...
      <div v-show="showModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 transition-opacity">
        <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
          ...
          <div v-if="modalMode==='view' && selectedUser" class="space-y-2">
            ...
            <p>Created At: <strong>{{ new Date(selectedUser.created_at).toLocaleString() }}</strong></p>
          </div>
          ...
          <form v-else @submit.prevent="submitUser" class="space-y-4">
            ...
            <div>
              ...
              <input v-model="form.email" type="email" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm" required />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700">Role</label>
              <select v-model="form.role_id" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm" required>
