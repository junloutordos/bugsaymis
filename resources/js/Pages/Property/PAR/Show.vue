...
<template>
  ...
  <AdminLayout :title="`PAR — ${par.par_number}`">
    <div class="flex flex-wrap gap-3 items-center mb-6">
      <span class="px-3 py-1 rounded-full text-sm font-medium" :class="statusColors[par.status]??'bg-slate-100 text-slate-600'">{{ par.status_label }}</span>
      <div class="ml-auto"><a :href="route('property.par.pdf',par.id)" target="_blank" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2"><DocumentArrowDownIcon class="h-4 w-4"/>Print PAR</a></div>
    </div>
