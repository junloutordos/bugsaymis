...
function addItem() { form.value.items.push({ property_item_id:'', description:'', unit:'', quantity:'', unit_cost:'' }) }
...
function onPropSelect(idx) {
  ...
  if (found) {
    ...
    form.value.items[idx].quantity = found.quantity
  }
}
...
async function submit() {
  ...
  try {
    await axios.post(route('property.par.store'), form.value)
    router.visit(route('property.par.index'))
  } catch(e) {
