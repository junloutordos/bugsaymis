...
const applyFilters = () => {
    ...
    router.get(route('facility-requests.fad-approval'), { search: search.value || undefined }, {
...
const clearFilters = () => {
  search.value = ''
  isLoading.value = true
  router.get(route('facility-requests.fad-approval'), {}, {
    preserveState: true, replace: true,
    only: ['requests', 'filters'],
    onFinish: () => { isLoading.value = false },
  })
...
const goToPage = (pageNum) => {
  isLoading.value = true
  router.get(route('facility-requests.fad-approval'), { search: search.value || undefined, page: pageNum }, {
    preserveState: true, replace: true,
    only: ['requests', 'filters'],
    onFinish: () => { isLoading.value = false },
  })
...
function handleApproveConfirm(pin) {
  ...
  router.post(route('facility-requests.fad-action', pendingApproveId.value), { action: 'approve', pin: pin || null }, {
    onSuccess: () => {
      const err = usePage().props.flash?.error
      if (err) Swal.fire('Not approved', err, 'error')
      else Swal.fire('Approved!', 'Facility request approved by FAD.', 'success')
    },
...
const rejectRequest = async (id) => {
  ...
  router.post(route('facility-requests.fad-action', id), { action: 'reject' }, {
...
<template>
  <Head title="FAD Approval - Facility Requests" />
  <AdminLayout title="FAD Approval - Facility Requests">
    <div>
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          <h1 class="text-xl font-semibold text-slate-800">FAD Approval — Facility Requests</h1>
          <p class="text-sm text-slate-500 mt-0.5">Facility requests approved by Division Chief, awaiting FAD review</p>
        </div>
      </div>
          ...
          <table class="min-w-full divide-y divide-slate-100 text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
              ...
              <tr v-if="filteredRequests.length === 0">
                <td colspan="7" class="py-16 text-center text-slate-400 text-sm">No facility requests pending FAD approval.</td>
              </tr>
            </tbody>
          </table>
        </div>
