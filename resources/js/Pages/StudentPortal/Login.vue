...
<template>
    ...
    <div class="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50 flex items-center justify-center px-4">
        <div class="w-full max-w-sm space-y-6">
            ...
            <!-- Flash messages -->
            <div v-if="flash.error" class="bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg px-4 py-3">
                {{ flash.error }}
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 space-y-5">
                ...
                <a
                    :href="route('student-portal.auth.google')"
                    class="flex items-center justify-center gap-3 w-full bg-white border border-slate-300 hover:bg-slate-50 hover:border-slate-400 text-slate-700 font-medium text-sm rounded-lg py-2.5 transition-colors shadow-sm"
                >
                    <!-- Google "G" logo -->
            ...
            <p class="text-xs text-center text-slate-400">
                First time? After signing in with Google, you'll be asked to enter your PISAY System ID to link your record.
            </p>
        </div>
    </div>
</template>
