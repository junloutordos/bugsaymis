<template>
  <Head title="FAD Approval - Service Requests" />
  <AdminLayout title="FAD Approval - Service Requests">
    <div class="space-y-5">

      <AppPageHeader title="FAD Approval — Service Requests" subtitle="Service requests approved by Division Chief, awaiting FAD review" />
      ...
      <AppTable :is-empty="filteredRequests.length === 0" :skeleton-cols="7">
        <template #head>
          <tr>
            ...
            <th class="px-4 py-3 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Service Type</th>
            <th class="px-4 py-3 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Date Needed</th>
            <th class="px-4 py-3 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Purposes</th>
        ...
        <template #empty>
          <EmptyState title="No service requests pending FAD approval." />
        </template>
...
const applyFilters = () => {
    ...
    router.get(route('service-requests.fad-approval'), { search: search.value || undefined }, {
...
const clearFilters = () => {
  search.value = ''
  isLoading.value = true
  router.get(route('service-requests.fad-approval'), {}, {
    preserveState: true, replace: true,
    only: ['requests', 'filters'],
    onFinish: () => { isLoading.value = false },
  })
...
const goToPage = (pageNum) => {
  isLoading.value = true
  router.get(route('service-requests.fad-approval'), { search: search.value || undefined, page: pageNum }, {
    preserveState: true, replace: true,
    only: ['requests', 'filters'],
    onFinish: () => { isLoading.value = false },
  })
...
const approveRequest = async (id) => {
  ...
  router.post(route('service-requests.fad-action', id), { action: 'approve' }, {
    onSuccess: () => Swal.fire('Approved!', 'Service request approved by FAD.', 'success'),
...
const rejectRequest = async (id) => {
  const result = await Swal.fire({
    title: 'Decline this service request?',
  ...
  router.post(route('service-requests.fad-action', id), { action: 'reject' }, {
    onSuccess: () => Swal.fire('Declined', 'Service request declined.', 'error'),
    onFinish:  () => { isSubmitting.value = false },
  })
}
