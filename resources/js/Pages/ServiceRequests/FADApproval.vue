  ...
  <AdminLayout title="FAD Approval - Service Requests">
    <div>
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          <h1 class="text-xl font-semibold text-slate-800">FAD Approval — Service Requests</h1>
          <p class="text-sm text-slate-500 mt-0.5">Service requests approved by Division Chief, awaiting FAD review</p>
        </div>
      </div>
          ...
          <table class="min-w-full divide-y divide-slate-100 text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
              <tr v-for="req in filteredRequests" :key="req.id" class="hover:bg-slate-50/60">
                ...
                <td class="px-4 py-3 text-center">
                  <div class="flex items-center gap-1.5 justify-center">
                    ...
                    <button @click="rejectRequest(req.id)" :disabled="isSubmitting"
              ...
              <tr v-if="filteredRequests.length === 0">
                <td colspan="7" class="py-16 text-center text-slate-400 text-sm">No service requests pending FAD approval.</td>
              </tr>
            </tbody>
          </table>
        </div>
