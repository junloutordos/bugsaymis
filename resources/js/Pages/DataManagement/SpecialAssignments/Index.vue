    ...
    <div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p class="text-sm text-slate-500 mt-0.5">Manage special assignments and coordinators</p>
        </div>
          ...
          <table class="min-w-full divide-y divide-slate-100 text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
              ...
              <tr v-if="filteredAssignments.length === 0">
                <td colspan="5" class="py-16 text-center text-slate-400 text-sm">No assignments found.</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div v-if="showModal" class="fixed inset-0 flex items-center justify-center bg-slate-900/50 z-50">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-screen overflow-y-auto">
          ...
          <!-- FORM -->
          <form v-else @submit.prevent="submitAssignment" class="px-6 py-5 space-y-4">
            <div>
