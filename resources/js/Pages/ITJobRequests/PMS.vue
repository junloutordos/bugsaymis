...
<template>
  ...
  <AdminLayout title="Preventive Maintenance Schedule">
    <div>
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-1">
                    ...
                    <button @click="openAssignEquipmentModal(s)"
                      class="p-1.5 rounded-lg hover:bg-emerald-50 text-emerald-600 hover:text-emerald-700 transition-colors" title="Assign Equipment">
      ...
      <div v-if="showAssignEquipmentModal" class="fixed inset-0 flex items-center justify-center bg-slate-900/50 z-50">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
          <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h2 class="text-base font-semibold text-slate-800">Assign Equipment to PMS Schedule</h2>
          ...
          <form @submit.prevent="assignEquipment" class="px-6 py-5 space-y-4">
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1">Select Equipment(s)</label>
              <multiselect
