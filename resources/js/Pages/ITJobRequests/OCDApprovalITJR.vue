...
const applyFilters = () => {
    ...
    router.get(route('job-requests.ocd-approval'), buildParams(1), {
      preserveState: true,
      replace: true,
...
const clearFilters = () => {
  ...
  isLoading.value = true
  router.get(route('job-requests.ocd-approval'), {}, {
    preserveState: true,
    replace: true,
    only: ['requests', 'filters'],
    onFinish: () => { isLoading.value = false },
...
const goToPage = (pageNum) => {
  ...
  isLoading.value = true
  router.get(route('job-requests.ocd-approval'), buildParams(page), {
    preserveState: true,
    replace: true,
    only: ['requests', 'filters'],
    onFinish: () => { isLoading.value = false },
