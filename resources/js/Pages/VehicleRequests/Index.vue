              ...
              <tr v-for="req in filteredRequests" :key="req.id" class="hover:bg-slate-50/60">
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-1 justify-center">
                    ...
                    <button v-if="hasAnyRole('Administrator','GSU Head') && ['Approved','FAD Approved','OCD Approved'].includes(req.status) && !req.driver" @click.prevent="openAssignDriverModal(req)" class="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 hover:text-indigo-600 transition-colors" title="Assign Driver"><UserIcon class="w-4 h-4" /></button>
        ...
        <div class="sm:hidden space-y-3 p-4">
          <div v-for="req in filteredRequests" :key="req.id" class="border border-slate-200 rounded-lg p-3 bg-white shadow-sm">
            ...
            <div class="mt-3 flex items-center gap-2">
              ...
              <button v-if="hasAnyRole('Administrator','GSU Head') && ['Approved','FAD Approved','OCD Approved'].includes(req.status) && !req.driver" @click.prevent="openAssignDriverModal(req)" class="inline-flex items-center gap-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-3 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm"><UserIcon class="w-4 h-4" /> Assign</button>
