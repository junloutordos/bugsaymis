...
const clearFilters = () => {
  search.value = ''
  isLoading.value = true
  router.get(route('vehicle-requests.ocd-approval'), {}, {
    preserveState: true, replace: true,
    only: ['requests', 'filters'],
    onFinish: () => { isLoading.value = false },
  })
...
<template>
  ...
  <AdminLayout title="OCD Approval - Vehicle Requests">
    <div class="space-y-5">
      ...
      <AppPageHeader title="OCD Approval — Vehicle Requests" subtitle="Review and act on vehicle requests pending OCD approval" />
      ...
      <AppTable :is-empty="!filteredRequests.length" :skeleton-cols="7">
        ...
        <template #empty>
          <EmptyState title="No pending vehicle requests for OCD approval" />
        </template>
