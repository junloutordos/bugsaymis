      ...
      >
        <AppTable :is-empty="cycles.length === 0" :skeleton-cols="6" :card="false">
          ...
          <tr
            ...
            :key="cycle.id"
            class="cursor-pointer transition-colors hover:bg-indigo-50/40 focus-within:bg-indigo-50/40"
            tabindex="0"
            @click="router.visit(route('alp.cycles.show', cycle.id))"
            @keydown.enter="router.visit(route('alp.cycles.show', cycle.id))"
          >
            <td :class="TD">
