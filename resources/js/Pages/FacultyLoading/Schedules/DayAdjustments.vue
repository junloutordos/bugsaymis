  ...
  <AdminLayout title="Adjusted Day Schedules">
    <div class="space-y-5">
      <AppPageHeader
        ...
        subtitle="Move a postponed Monday flag ceremony to the next common school day without changing the regular weekly timetable."
      >
        <template #actions>
      ...
      <AppCard v-if="!adjustments.length">
        <EmptyState title="No adjusted school days" subtitle="Create one when a Monday flag ceremony must move because of a campus-wide holiday or class suspension." :icon="CalendarDaysIcon" />
      </AppCard>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
              <tr>
                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Affected Monday</th>
                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Adjusted day</th>
            ...
            <tbody class="divide-y divide-slate-100 bg-white">
              <tr v-for="item in adjustments" :key="item.id">
                <td class="px-4 py-3 text-sm text-slate-700">{{ formatDate(item.postponed_from_date) }}</td>
                <td class="px-4 py-3 text-sm">
                  <div class="font-semibold text-slate-800">{{ item.weekday }}, {{ formatDate(item.effective_date) }}</div>
                  <div class="text-xs text-slate-500">Flag 7:30–8:00 AM · Classes shifted {{ item.shift_minutes }} minutes</div>
                </td>
    ...
    <AppModal :show="showForm" :title="editingId ? 'Edit Adjusted Day' : 'New Adjusted Day'" size="lg" @close="showForm = false">
      <div class="space-y-4">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Next common school day</label>
            <input v-model="form.effective_date" type="date" readonly
        ...
        <div>
          ...
          <textarea v-model="form.reason" rows="3" placeholder="Example: Monday classes suspended due to Typhoon…"
        ...
        <div class="grid grid-cols-2 gap-3 rounded-lg bg-slate-50 p-3 text-sm">
          <div><span class="text-slate-500">Flag ceremony:</span> <strong>7:30–8:00 AM</strong></div>
          <div><span class="text-slate-500">Uniform shift:</span> <strong>30 minutes</strong></div>
        </div>
      </div>
