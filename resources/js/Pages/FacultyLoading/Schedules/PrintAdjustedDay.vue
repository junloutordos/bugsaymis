    ...
    <main class="sheet-body">
      <header class="document-heading">
        <div>
          ...
          <p>{{ formattedEffectiveDate }} · {{ term.label }}</p>
        </div>
        <div class="validity-box"><strong>VALID ONLY FOR</strong><span>{{ shortEffectiveDate }}</span></div>
      </header>
          ...
          <div class="section-columns">
            ...
            <div v-for="section in grade.sections" :key="section.id" class="section-column">
              <div class="ceremony-block" :style="rangeStyle(snapshot.ceremony.start, snapshot.ceremony.end)">
                <strong>Flag Ceremony</strong><span>{{ formatRange(snapshot.ceremony.start, snapshot.ceremony.end) }}</span>
              </div>
              <div v-for="band in section.bands" :key="`${band.type}-${band.start}-${band.end}`"
                ...
                <strong>{{ band.label }}</strong><span>{{ formatRange(band.start, band.end) }}</span>
              </div>
              <div v-for="entry in section.entries" :key="entry.id" class="class-entry"
                ...
                <span v-if="duration(entry) >= 40">{{ entry.classroom?.code ?? entry.classroom?.name ?? '' }}</span>
                <small>{{ formatRange(entry.start_time, entry.end_time) }}</small>
              </div>
            </div>
...
const startMinutes = computed(() => timeToMinutes(props.snapshot.calendar_start))
const endMinutes = computed(() => timeToMinutes(props.snapshot.calendar_end))
...
const shortEffectiveDate = computed(() => new Date(`${props.adjustment.effective_date}T00:00:00`).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' }))
...
.document-heading { display: flex; align-items: center; justify-content: space-between; border-bottom: .5mm solid #1e3a8a; padding: 1.5mm 0; }
.document-heading h1 { margin: 0; font-size: 14pt; letter-spacing: .04em; color: #172554; }
.document-heading p { margin: 1mm 0 0; font-size: 8pt; color: #475569; }
.validity-box { display: flex; flex-direction: column; align-items: center; border: .3mm solid #dc2626; padding: 1.2mm 3mm; color: #b91c1c; }
.validity-box strong { font-size: 6.5pt; letter-spacing: .12em; }
.validity-box span { font-size: 8pt; font-weight: 700; }
...
.class-entry small { margin-top: .3mm; font-size: 5.7pt; font-weight: 700; }
