...
<template>
  ...
  <AdminLayout title="New Inspection & Acceptance Report">
    <div class="space-y-5">
      ...
      <AppPageHeader title="New Inspection & Acceptance Report" subtitle="Record a delivery for inspection and acceptance into stock." />

      <form @submit.prevent="submit" class="space-y-6">
        <!-- Header Info -->
        <AppCard title="IAR Details">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1">Purchase Order (optional)</label>
            ...
            <AppInput v-model="form.supplier_name" type="text" label="Supplier Name" required :error="errors.supplier_name?.[0]" />
            <AppInput v-model="form.supplier_address" type="text" label="Supplier Address" :error="errors.supplier_address?.[0]" />
            <AppInput v-model="form.supplier_tin" type="text" label="TIN" :error="errors.supplier_tin?.[0]" />
            <AppInput v-model="form.delivery_date" type="date" label="Delivery Date" required :error="errors.delivery_date?.[0]" />
            <AppInput v-model="form.inspection_date" type="date" label="Inspection Date" :error="errors.inspection_date?.[0]" />
            <div>
            ...
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1">Property Officer</label>
            ...
            <div class="md:col-span-2">
              <AppTextarea v-model="form.remarks" :rows="2" label="Remarks" :error="errors.remarks?.[0]" />
            </div>
          </div>
        </AppCard>

        <!-- Items -->
        <AppCard title="Items Received">
          <template #header>
          ...
          <div v-for="(item, idx) in form.items" :key="idx"
            ...
            <button type="button" @click="removeItem(idx)"
              class="absolute top-3 right-3 text-danger-500 hover:text-danger-700">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div class="md:col-span-2">
                <AppInput v-model="item.description" type="text" label="Description" required />
              </div>
              <AppInput v-model="item.unit" type="text" label="Unit" required />
              <AppInput v-model="item.quantity_ordered" type="number" step="0.001" min="0.001" label="Qty Ordered" required />
              <AppInput v-model="item.quantity_delivered" type="number" step="0.001" min="0" label="Qty Delivered" required />
              <AppInput v-model="item.unit_cost" type="number" step="0.01" min="0" label="Unit Cost" required />
            </div>
          </div>
        </AppCard>

        <div class="flex justify-end gap-3">
          <AppButton as="link" variant="secondary" :href="route('supply.iar.index')">Cancel</AppButton>
