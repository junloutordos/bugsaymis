...
const applyFilters = () => {
    ...
    router.get(route('work-requests.fad-approval'), { search: search.value || undefined }, {
...
const clearFilters = () => {
  search.value = ''
  isLoading.value = true
  router.get(route('work-requests.fad-approval'), {}, {
    preserveState: true, replace: true,
...
const goToPage = (pageNum) => {
  isLoading.value = true
  router.get(route('work-requests.fad-approval'), { search: search.value || undefined, page: pageNum }, {
    preserveState: true, replace: true,
    only: ['workRequests', 'filters'],
    onFinish: () => { isLoading.value = false },
  })
...
const approveRequest = async (id) => {
  ...
  router.post(route('work-requests.fad-action', id), { action: 'approve' }, {
    onSuccess: () => Swal.fire('Approved!', 'Work request approved by FAD.', 'success'),
...
const rejectRequest = async (id) => {
  const confirmed = await confirmAction({
    title: 'Decline this work request?',
  ...
  router.post(route('work-requests.fad-action', id), { action: 'reject' }, {
    onSuccess: () => Swal.fire('Declined', 'Work request declined.', 'error'),
    onFinish:  () => { isSubmitting.value = false },
  })
}
</script>
...
<template>
  <Head title="FAD Approval - Work Requests" />
  <AdminLayout title="FAD Approval - Work Requests">
    <div class="space-y-5">

      <AppPageHeader title="FAD Approval — Work Requests" subtitle="Work requests approved by GSU Head, awaiting FAD review" />
      ...
      <AppTable :is-empty="filteredItems.length === 0" :skeleton-cols="7">
        <template #head>
          <tr>
            ...
            <th class="px-4 py-3 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Issue</th>
        ...
        <tr v-for="wr in filteredItems" :key="wr.id" class="hover:bg-indigo-50/40">
          ...
          <td class="px-4 py-3 text-center">
            <div class="flex items-center gap-1.5 justify-center">
              ...
              <AppButton size="sm" variant="danger" :disabled="isSubmitting" @click="rejectRequest(wr.id)">
        ...
        <template #empty>
          <EmptyState title="No work requests pending FAD approval." />
        </template>
