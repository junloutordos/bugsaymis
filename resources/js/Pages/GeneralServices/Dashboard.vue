...
function statusClass(status) {
  ...
  if (bucket === 'declined') return 'bg-rose-50 text-rose-700 ring-rose-100'
  ...
  <AdminLayout title="General Services Dashboard">
    <div class="space-y-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 class="text-xl font-semibold text-slate-900">General Services Dashboard</h1>
          <p class="mt-1 text-sm text-slate-500">Operational analytics for vehicle, facility, work, and service requests.</p>
        </div>
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div v-for="card in kpiCards" :key="card.label" class="rounded-lg border border-slate-100 bg-white p-5 shadow-sm">
      ...
      <div class="grid grid-cols-1 gap-5 xl:grid-cols-3">
        <div class="rounded-lg border border-slate-100 bg-white p-5 shadow-sm xl:col-span-2">
          <div class="mb-4 flex items-center justify-between">
            <div>
              <p class="text-sm font-semibold text-slate-800">Monthly Request Trend</p>
              <p class="text-xs text-slate-500">Last 12 months by request type</p>
            </div>
          </div>
          <div class="h-72">
        ...
        <div class="rounded-lg border border-slate-100 bg-white p-5 shadow-sm">
          <div class="mb-4">
            <p class="text-sm font-semibold text-slate-800">Status Mix</p>
            <p class="text-xs text-slate-500">Top status groups across services</p>
          </div>
          <div class="h-72">
      ...
      <div class="grid grid-cols-1 gap-5 xl:grid-cols-2">
        <div class="rounded-lg border border-slate-100 bg-white p-5 shadow-sm">
          <div class="mb-4">
            <p class="text-sm font-semibold text-slate-800">Module Workload</p>
            <p class="text-xs text-slate-500">Active, completed, and declined totals</p>
          </div>
          <div class="h-72">
        ...
        <div class="rounded-lg border border-slate-100 bg-white p-5 shadow-sm">
          <div class="mb-4">
            <p class="text-sm font-semibold text-slate-800">Work Request Categories</p>
            <p class="text-xs text-slate-500">Skilled personnel workload by category</p>
          </div>
          <div class="h-72">
          ...
          <div class="space-y-3">
            <a v-for="item in upcomingSchedule" :key="item.id" :href="item.extendedProps.url" class="block rounded-lg border border-slate-100 p-3 hover:bg-slate-50">
              <div class="flex items-start justify-between gap-3">
                ...
                <span :class="['shrink-0 rounded-full px-2 py-0.5 text-[11px] font-medium ring-1', statusClass(item.extendedProps.status)]">{{ item.extendedProps.status }}</span>
              </div>
      ...
      <div v-if="selectedEvent" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" @click.self="selectedEvent = null">
        <div class="w-full max-w-lg rounded-lg bg-white shadow-xl">
          <div class="border-b border-slate-100 px-5 py-4">
            <div class="flex items-start justify-between gap-4">
              <div>
                <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ selectedEvent.module }}</p>
                <h2 class="mt-1 text-lg font-semibold text-slate-900">{{ selectedEvent.title }}</h2>
              </div>
              <button class="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-700" @click="selectedEvent = null">x</button>
            </div>
          </div>
          <div class="space-y-3 px-5 py-4 text-sm">
            ...
            <div class="flex justify-between gap-4">
              ...
              <span :class="['rounded-full px-2 py-0.5 text-xs font-medium ring-1', statusClass(selectedEvent.status)]">{{ selectedEvent.status }}</span>
            </div>
            <div class="flex justify-between gap-4">
          ...
          <div class="flex justify-end gap-2 border-t border-slate-100 px-5 py-4">
            <button class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50" @click="selectedEvent = null">Close</button>
            <a :href="selectedEvent.url" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">Open Request</a>
          </div>
        </div>
      </div>
    </div>
