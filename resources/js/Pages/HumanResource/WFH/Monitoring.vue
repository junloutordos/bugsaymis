    ...
    <div class="p-6 space-y-5">
      ...
      <div class="bg-white rounded-xl shadow p-4 flex flex-wrap gap-3 items-end">
        ...
        <button
          @click="load"
          :disabled="loading"
          class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-sm rounded-lg font-medium transition"
        >
      ...
      <div class="bg-white rounded-xl shadow overflow-hidden">
        <div v-if="!filteredRows.length && !loading" class="text-center py-12 text-gray-400">
        ...
        <div v-else class="overflow-x-auto">
          <table class="min-w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                <th class="px-4 py-3 text-left font-semibold text-gray-600 w-6"></th>
                <th class="px-4 py-3 text-left font-semibold text-gray-600">Employee</th>
                <th class="px-4 py-3 text-left font-semibold text-gray-600">Date</th>
                <th class="px-4 py-3 text-left font-semibold text-gray-600">Time In</th>
                <th class="px-4 py-3 text-left font-semibold text-gray-600">Time Out</th>
                <th class="px-4 py-3 text-left font-semibold text-gray-600">Duration</th>
                <th class="px-4 py-3 text-center font-semibold text-gray-600">Accomplishments</th>
                <th class="px-4 py-3 text-center font-semibold text-gray-600">Status</th>
                <th class="px-4 py-3 text-center font-semibold text-gray-600">Location</th>
                <th class="px-4 py-3 text-center font-semibold text-gray-600">Photos</th>
              </tr>
            </thead>
            <tbody>
              <template v-for="row in filteredRows" :key="row.id">
                ...
                >
                  ...
                  <td class="px-4 py-3 font-medium text-gray-800">
                    {{ row.user?.name ?? '—' }}
                    <div class="text-xs text-gray-400 font-normal">{{ row.user?.position ?? '' }}</div>
                  </td>

                  <td class="px-4 py-3 text-gray-700">{{ formatDate(row.date) }}</td>
                  <td class="px-4 py-3 text-green-600 font-medium">{{ formatTime(row.time_in) }}</td>
                  <td class="px-4 py-3 text-blue-600 font-medium">{{ formatTime(row.time_out) }}</td>
                  <td class="px-4 py-3 text-gray-600">{{ calcDuration(row.time_in, row.time_out) }}</td>

                  <td class="px-4 py-3 text-center">
                    <span class="inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold"
                          :class="row.accomplishments?.length ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-400'">
                    ...
                    <div class="flex items-center justify-center gap-2">
                      <a v-if="row.time_in_photo_link"
                        ...
                        <img :src="driveThumb(row.time_in_photo_link)"
                             class="w-10 h-10 rounded-lg object-cover border-2 border-green-300"
                             alt="In" />
                      </a>
                ...
                <tr v-if="expanded === row.id" class="bg-indigo-50">
                  <td colspan="10" class="px-8 pb-4 pt-2">
                    ...
                    <div v-if="!row.accomplishments?.length" class="text-sm text-gray-400">
                    ...
                    <ul v-else class="space-y-2">
                      ...
                      >
                        <div class="flex-1 min-w-0">
                          <p class="font-semibold text-gray-800 text-sm">{{ acc.title }}</p>
                          <p v-if="acc.description" class="text-xs text-gray-500 mt-0.5 line-clamp-2">
                            {{ acc.description }}
                          </p>
                        </div>
                        ...
                        <div class="flex-shrink-0">
                          ...
                          <span v-else class="text-xs text-gray-300">No proof</span>
                        </div>
                      </li>
                    </ul>
                  </td>
        ...
        <div v-if="meta.last_page > 1"
          ...
          <div class="flex gap-2">
            <button
              ...
              class="px-3 py-1 rounded border border-gray-200 disabled:opacity-40 hover:bg-gray-50 transition"
            >Prev</button>
            <button
              @click="changePage(meta.current_page + 1)"
...
const SummaryCard = {
  ...
  computed: {
    textColor() {
      return { indigo: 'text-indigo-600', green: 'text-green-600', yellow: 'text-yellow-600' }[this.color] ?? 'text-gray-800'
    },
  },
}
