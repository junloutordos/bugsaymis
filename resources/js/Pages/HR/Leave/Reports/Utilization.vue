...
<template>
  ...
  <AdminLayout title="Leave Utilization Report">
    <div class="space-y-5">
      ...
      <div>
        <h1 class="text-xl font-semibold text-slate-800">Leave Utilization Report</h1>
        <p class="text-sm text-slate-500 mt-0.5">Per-employee leave balance and usage rate for {{ year }}.</p>
      </div>
      ...
      <div v-if="aggregates.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div v-for="agg in aggregates" :key="agg.leave_type_id"
      ...
      <div class="bg-white rounded-xl border border-slate-100 shadow-sm">
        <div class="px-5 py-4 border-b border-slate-100">
          <h2 class="text-sm font-semibold text-slate-800">Employee Detail</h2>
          <p class="text-xs text-slate-400 mt-0.5">{{ total }} record(s)</p>
        </div>
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-slate-100 text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
              ...
              <tr v-if="!creditData.length">
                <td colspan="9" class="py-12 text-center text-slate-400 text-sm">No credit records found for the selected filters.</td>
              </tr>
            </tbody>
          </table>
        </div>
