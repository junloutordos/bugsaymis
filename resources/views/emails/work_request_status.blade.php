...
<head>
  <meta charset="utf-8">
  <title>Work Request Status</title>
  <style>
  ...
  <div class="container">
    <div class="card">
      <div class="card-header">
        <h1>Work Request — {{ $status }}</h1>
      </div>
      <div class="card-body">
        <p class="lead">Hello {{ $request->requester?->name ?? 'Requestor' }},</p>
        <p>Your work request (ID: {{ $request->id }}) has been {{ strtolower($status) }}{{ $approver ? ' by ' . $approver : '' }}.</p>
