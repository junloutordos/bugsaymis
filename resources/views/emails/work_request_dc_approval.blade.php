...
@section('header-title','Work Request — Division Chief Approval Needed')
@section('header-subtitle','Atlas — GSU Work Requests')

@section('content')
<p class="greeting">Hello <strong>{{ $chief?->name ?? 'Division Chief' }}</strong>,</p>
<p class="lead">A work request has been submitted by a member of your division and requires your approval before it proceeds to GSU for assignment.</p>

<table class="details" role="presentation">
    <tr><td class="lbl">Request ID</td><td class="val"><strong>#{{ $request->id }}</strong></td></tr>
    <tr><td class="lbl">Requested By</td><td class="val">{{ $request->requester?->name ?? '—' }}</td></tr>
