...
<body>
  <div class="center">
    <div class="card" id="approvedCard">
      ...
      <h1>{{ $already ? 'Request Already Approved' : 'Purchase Request Approved' }}</h1>
      <p class="meta">{{ $already ? 'This request was already approved.' : 'Thank you — the purchase request has been approved successfully.' }}</p>
      <p class="meta">Request ID: {{ $procurement->id }}</p>
    ...
    (function(){
      ...
      try {
        if (window.opener && !window.opener.closed) {
          window.opener.postMessage({ type: 'procurementApproved', id: id, already: {{ $already ? 'true' : 'false' }} }, '*');
        }
      } catch (e){ }
      setTimeout(() => {
      ...
      document.getElementById('returnBtn')?.addEventListener('click', function(){ try { if (window.opener && !window.opener.closed) window.opener.postMessage({ type: 'procurementApproved', id: id }, '*'); } catch(e){} setTimeout(() => { try { window.close(); } catch(e){} }, 200); });
    })();
  </script>
</body>
</html>
