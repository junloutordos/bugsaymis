...
<div class="form-no">GAM Form No. 20</div>
<h2>PROPERTY ACKNOWLEDGMENT RECEIPT (PAR)</h2>
<div class="subtitle">Philippine Science High School – Caraga Region Campus</div>
<table class="header-table">
  <tr>
    <td width="50%"><strong>PAR No.: {{ $par->par_number }}</strong></td>
    <td width="50%" style="text-align:right"><strong>Date: {{ $par->issue_date?->format('F j, Y') }}</strong></td>
  </tr>
  <tr>
    <td>Division/Office: {{ $par->division?->division_name ?? '—' }}</td>
    <td style="text-align:right">Issued By: {{ $par->issuer?->name ?? '—' }}</td>
  </tr>
</table>
<table class="items">
  <thead>
  ...
  <tbody>
    ...
    @endforeach
    @for($j=count($par->items);$j<6;$j++)<tr><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>@endfor
    <tr style="font-weight:bold;background:#f9f9f9">
      <td colspan="6" class="right">TOTAL:</td>
      <td class="right">{{ number_format($par->totalAmount(),2) }}</td><td></td>
    </tr>
  </tbody>
</table>
<table class="signatures">
  <tr>
    <td><strong>ISSUED BY:</strong>
      ...
      <strong>{{ $par->issuer?->name ?? '________________________' }}</strong><br>
      <span style="font-size:7.5pt;color:#555">Supply / Property Officer</span>
    </td>
    <td><strong>RECEIVED BY:</strong>
      <div class="sig-line"></div>
      <strong>{{ $par->receiver?->name ?? '________________________' }}</strong><br>
      <span style="font-size:7.5pt;color:#555">Accountable Officer</span><br>
      <span style="font-size:7.5pt;color:#555">Date: ________________</span>
    </td>
  </tr>
</table>
<div class="footer">Generated: {{ now()->format('F j, Y g:i A') }} | GAM Form No. 20 (COA Circular 2015-010)</div>
</body></html>
