...
<body>
  <div class="center">
    <div class="card" id="approvedCard">
      ...
      <h1>{{ $already ? 'Request Already Approved' : 'Gate Pass Approved' }}</h1>
      <p class="meta">{{ $already ? 'This gate pass was already approved.' : 'Thank you — the gate pass has been approved successfully.' }}</p>
      <p class="meta">Gate Pass ID: {{ $gatepass->id }}</p>
    ...
    (function(){
      ...
      try {
        if (window.opener && !window.opener.closed) {
          window.opener.postMessage({ type: 'gatepassApproved', id: id, already: {{ $already ? 'true' : 'false' }} }, '*');
        }
      } catch (e){ }

      @if(!empty($already))
        try { alert('This gate pass has already been approved.'); } catch(e){}
      @endif

      const tryClose = () => { try { window.close(); } catch (e) {} }
      setTimeout(() => { tryClose(); setTimeout(() => { window.location.href = '{{ url('/') }}'; }, 1200); }, 2200);
      document.getElementById('returnBtn')?.addEventListener('click', function(){ try { if (window.opener && !window.opener.closed) window.opener.postMessage({ type: 'gatepassApproved', id: id }, '*'); } catch(e){}; setTimeout(tryClose, 200); });
    })();
  </script>
</body>
</html>
