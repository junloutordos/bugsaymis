...
<div class="meta">S.Y. {{ $schoolYearName }} | {{ count($members) }} member(s)@if($filterLabel) | Filtered: {{ $filterLabel }}@endif</div>
<table><thead><tr><th>#</th><th>Name</th><th>Grade</th><th>Section</th><th>ALP</th></tr></thead><tbody>
@forelse($members as $row)<tr><td>{{ $loop->iteration }}</td><td>{{ $row['name'] }}</td><td>{{ $row['grade_level'] }}</td><td>{{ $row['section'] }}</td><td>{{ $row['alp'] }}</td></tr>@empty
<tr><td colspan="5" class="no-data">No active members found.</td></tr>@endforelse
</tbody></table>
