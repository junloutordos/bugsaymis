...
<body>
  ...
  <script>
    (function(){
      const id = {{ $facilityRequest->id }};
      try {
        if (window.opener && !window.opener.closed) {
          window.opener.postMessage({ type: 'workRequestApproved', id: id, already: {{ $already ? 'true' : 'false' }} }, '*');
        }
      } catch (e){ }
      ...
      document.getElementById('returnBtn')?.addEventListener('click', function(){ try { if (window.opener && !window.opener.closed) window.opener.postMessage({ type: 'workRequestApproved', id: id }, '*'); } catch(e){} setTimeout(tryClose, 200); });
    })();
  </script>
</body>
</html>
