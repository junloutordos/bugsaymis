...
$sheet->setCellValue('M14', strtolower($dualType) === 'by naturalization' ? '☑ By Naturalization' : '☐ By Naturalization');
    ...
    foreach ($pds->eligibility ?? [] as $e) {
        ...
        $sheet->setCellValue("G{$row}", $e->exam_date ?? '');
        $sheet->setCellValue("I{$row}", $e->place_taken ?? '');
        $sheet->setCellValue("J{$row}", $e->license_number ?? '');
        $sheet->setCellValue("N{$row}", $e->license_validity ?? '');
        $row++;
    }

    $row = 18;
    foreach ($pds->workExperience ?? [] as $w) {
        $sheet->setCellValue("A{$row}", $w->from_date ?? '');
        $sheet->setCellValue("C{$row}", $w->to_date ?? '');
    ...
    foreach ($pds->voluntaryWork ?? [] as $v) {
        ...
        $sheet->setCellValue("D{$row}", $v->from_date ?? '');
        $sheet->setCellValue("G{$row}", $v->to_date ?? '');
    ...
    foreach ($pds->trainings ?? [] as $t) {
        ...
        $sheet->setCellValue("E{$row}", $t->date_from ?? '');
        $sheet->setCellValue("F{$row}", $t->date_to ?? '');
